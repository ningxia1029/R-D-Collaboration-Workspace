...
本地环境：Windows / Node.js 24.12.0 / WSL Ubuntu 22.04 / PostgreSQL 14.23 隔离实例
...
- deterministic 微基准不代表真实性能；DeepSeek live 指标包含真实模型和网络，但只使用合成 Tool 执行器，不包含 PostgreSQL、Web/SSE 或完整产品端到端延迟。
...
- DeepSeek V4 Flash 非思考 Provider/Harness 已完成脱敏基线；尚未完成思考模式、隔离 Web→Worker→真实 Tool Gateway→数据库/SSE 全链路、企业 IdP、生产域名/证书、SIEM、正式代码签名证书和两版可回退安装包。
