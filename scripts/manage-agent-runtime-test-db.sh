...
# 仅管理 /tmp/workbuddy-phase3-* 下的可丢弃验收实例，拒绝其他路径。
ACTION="${1:-}"
CLUSTER_ROOT="${2:-}"
PORT="${3:-}"
DATABASE_NAME="${4:-}"
...
if [[ ! "${DATABASE_NAME}" =~ ^workbuddy_phase3_[a-zA-Z0-9_]+$ ]]; then
...
case "${ACTION}" in
  start)
    ...
    "${POSTGRES_BIN}/initdb" \
      -D "${CLUSTER_ROOT}/data" \
      --auth=trust \
      --username=postgres >"${CLUSTER_ROOT}/initdb.log"
    if ! "${POSTGRES_BIN}/pg_ctl" \
      -D "${CLUSTER_ROOT}/data" \
      -l "${CLUSTER_ROOT}/postgres.log" \
      -o "-h 127.0.0.1 -p ${PORT} -k ${CLUSTER_ROOT}" \
      -w start; then
      echo "PostgreSQL 临时实例启动日志：" >&2
    ...
    echo "PHASE3_CLUSTER=${CLUSTER_ROOT}"
    echo "PHASE3_DB=${DATABASE_NAME}"
    ;;
  stop)
    ...
    [[ ! -e "${CLUSTER_ROOT}" ]]
    echo "PHASE3_CLEANUP_OK"
    ;;
  *)
    echo "用法：$0 {start|stop} /tmp/workbuddy-phase3-* PORT workbuddy_phase3_NAME" >&2
    exit 64
    ;;
esac
