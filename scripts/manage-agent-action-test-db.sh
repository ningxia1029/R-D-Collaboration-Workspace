...
# 仅管理 /tmp/workbuddy-phase7-* 下的可丢弃验收实例，拒绝其他路径。
ACTION="${1:-}"
CLUSTER_ROOT="${2:-}"
PORT="${3:-}"
DATABASE_NAME="${4:-}"
...
if [[ ! "${DATABASE_NAME}" =~ ^workbuddy_phase7_[a-zA-Z0-9_]+$ ]]; then
...
case "${ACTION}" in
  start)
    ...
    echo "PHASE7_CLUSTER=${CLUSTER_ROOT}"
    echo "PHASE7_DB=${DATABASE_NAME}"
    ;;
  stop)
    ...
    [[ ! -e "${CLUSTER_ROOT}" ]]
    echo "PHASE7_CLEANUP_OK"
    ;;
  *)
    echo "用法：$0 {start|stop} /tmp/workbuddy-phase7-* PORT workbuddy_phase7_NAME" >&2
    exit 64
    ;;
esac
